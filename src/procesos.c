 ...
 * r: puntero a la estructura resultado_t con los datos del grupo.
 * Si el archivo tiene 8 o más líneas, lo reescribe; si no, agrega al final.
 */
static void escribir_resultado(const char *filename, const resultado_t *r)
{
    int rewrite = 0;
    // Solo reescribe si el archivo tiene más de 9 líneas
    FILE *f = fopen(filename, "r");
    if (f) {
        int lines = 0;
        char buffer[256];
        while (fgets(buffer, sizeof(buffer), f)) lines++;
        if (lines > 11) rewrite = 1;
        fclose(f);
    }
    ...
    if (f) {
        fprintf(f,
                ...
                r->letra, r->promedio, r->reprobados, r->aprobado_bajo, r->aprobado_alto, r->tiempo);
        fflush(f);
        fclose(f);
    }
}
...
int main(void)
{
    ...
    sem_t *sem = sem_open(SEM_NAME, O_CREAT, 0666, 1); // SEM_NAME: nombre, O_CREAT: crear si no existe, permisos 0666, valor inicial 1
    ...
    int shm_fd = shm_open("/resumen_global", O_CREAT | O_RDWR, 0666); // /resumen_global: nombre, O_CREAT|O_RDWR: crear y leer/escribir, permisos 0666
    ftruncate(shm_fd, 3 * sizeof(int)); // Ajusta el tamaño de la memoria compartida
    int *resumen_global = mmap(0, 3 * sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0); // Mapea la memoria compartida
    ...
    for (int g = 0; g < N_GRUPOS; ++g) {
        ...
        if (pid < 0) { perror("fork"); return EXIT_FAILURE; }
            ...
            // Llena la estructura resultado_t con los datos del grupo
            resultado_t r = { .letra = 'A' + g,
            ...
            sem_wait(sem); // Espera el semáforo antes de modificar el recurso compartido
            ...
            sem_close(sem); // Cierra el semáforo
            munmap(resumen_global, 3 * sizeof(int)); // Libera la memoria compartida
            close(shm_fd); // Cierra el descriptor de la memoria compartida
    ...
    escribir_tiempos_finales("resultados_procesos.txt", dt, tiempo_inicio, tiempo_fin, t0, t1);
    ...
    close(shm_fd); // Cierra descriptor de memoria compartida
